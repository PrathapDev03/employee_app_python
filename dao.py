...
def add_employee(emp: Employee) -> bool:
    sql = "INSERT INTO emp (id, firstName, lastName, salary, designation) VALUES (%s, %s, %s, %s, %s)"
    conn = get_connection()
    try:
...
def update_employee(emp: Employee) -> bool:
    sql = "UPDATE emp SET firstName=%s, lastName=%s, salary=%s, designation=%s WHERE id=%s"
    conn = get_connection()
    try:
        with conn.cursor() as cur:
...
def get_employee_by_id(emp_id: int) -> Optional[Employee]:
    sql = "SELECT id, firstName, lastName, salary, designation FROM emp WHERE id=%s"
    conn = get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(sql, (emp_id,))
